...
struct EpipolarConsensusError {
    ...
    bool operator()(const T* const camera, T* residuals) const {
        ...
        T R_kq[9];
        R_kq[0] = T(cam_k[0]) * R_wq[0] + T(cam_k[1]) * R_wq[1] + T(cam_k[2]) * R_wq[2];
        R_kq[1] = T(cam_k[3]) * R_wq[0] + T(cam_k[4]) * R_wq[1] + T(cam_k[5]) * R_wq[2];
        R_kq[2] = T(cam_k[6]) * R_wq[0] + T(cam_k[7]) * R_wq[1] + T(cam_k[8]) * R_wq[2];

        R_kq[3] = T(cam_k[0]) * R_wq[3] + T(cam_k[1]) * R_wq[4] + T(cam_k[2]) * R_wq[5];
        R_kq[4] = T(cam_k[3]) * R_wq[3] + T(cam_k[4]) * R_wq[4] + T(cam_k[5]) * R_wq[5];
        R_kq[5] = T(cam_k[6]) * R_wq[3] + T(cam_k[7]) * R_wq[4] + T(cam_k[8]) * R_wq[5];

        R_kq[6] = T(cam_k[0]) * R_wq[6] + T(cam_k[1]) * R_wq[7] + T(cam_k[2]) * R_wq[8];
        R_kq[7] = T(cam_k[3]) * R_wq[6] + T(cam_k[4]) * R_wq[7] + T(cam_k[5]) * R_wq[8];
        R_kq[8] = T(cam_k[6]) * R_wq[6] + T(cam_k[7]) * R_wq[7] + T(cam_k[8]) * R_wq[8];


        T t_kq[3];
        t_kq[0] = T(cam_k[9]) - (R_kq[0] * camera[3] + R_kq[1] * camera[4] * R_kq[2] * camera[5]);
        t_kq[1] = T(cam_k[10]) - (R_kq[3] * camera[3] + R_kq[4] * camera[4] * R_kq[5] * camera[5]);
        t_kq[2] = T(cam_k[11]) - (R_kq[6] * camera[3] + R_kq[7] * camera[4] * R_kq[8] * camera[5]);
        ...
        T l[3];
        for (int i = 0; i < matches_k.size(); i++) {
            ...
            residuals[i] = l[0]*T(matches_k[i][2]) + l[1]*T(matches_k[i][3]) + l[2];
        }
    ...
    static ceres::CostFunction* Create(const vector<double>& cam_k,
        ...
        return (new ceres::AutoDiffCostFunction<EpipolarConsensusError, ceres::DYNAMIC, 9> (
                new EpipolarConsensusError(cam_k, matches_k), matches_k.size()));
