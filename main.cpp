...
//
    ...
    ofstream R_qk_to_gt;
    R_qk_to_gt.open ("/Users/cameronfiore/C++/image_localization_project/data/analysis/rel_pose_analysis/R_qk_to_gt.txt");
    ofstream T_qk_to_gt;
    T_qk_to_gt.open ("/Users/cameronfiore/C++/image_localization_project/data/analysis/rel_pose_analysis/T_qk_to_gt.txt");
...
//        double K[4] = {2584.9325098195013197, 2584.7918606057692159, 249.77137587221417903, 278.31267937919352562};
        ...
        auto retrieved = functions::retrieveSimilar(query, ".color.png", 150, 1.6);
...
//            cambridge::getAbsolutePose(im, R_k, t_k);
            ...
            functions::findMatches(0.4, desc_i, kp_i, desc_q, kp_q, pts_i, pts_q);
            if (pts_i.size() < 5) continue;

//            cv::Mat src;
//            cv::Mat q_mat = cv::imread(query+EXT);
//            cv::Mat im_mat = cv::imread(im+EXT);
...
//                circle(q_mat, pt_q, 3, color, -1);
//                circle(im_mat, pt_i, 3, color, -1);
//                cv::line(src, pt_q, pt_i, color, 1);
//            }
...
//            synthetic::addMismatches(pts_db, pts_q, .10);
            ...
            functions::getRelativePose(pts_i_qk, pts_q_qk, K, R_qk_calc, T_qk_calc);

            vector<cv::Point2d> pts_q_kq = pts_q, pts_i_kq = pts_i;
            functions::getRelativePose(pts_q_kq, pts_i_kq, K, R_kq_calc, T_kq_calc);
            ...
            if (r_consistency >= 0.001 || t_consistency >= 0.001) {
                continue;
            }
        ...
        int num_inliers = int (get<8>(results).size());

        double c_ = functions::getDistBetween(get<0>(results), c_q);

        double T_gov_ = functions::getDistBetween(get<1>(results), T_q);
        double T_cf_ = functions::getDistBetween(get<2>(results), T_q);

        double R_avg_ = functions::rotationDifference(get<3>(results), R_q);
...
//            functions::findMatches(im, query, ".color.png",  "SIFT", 0.8, pts_db, pts_q);
//            if (pts_db.size() < 5) continue;
//
//            Eigen::Matrix3d R_qk;
//            Eigen::Vector3d T_qk;
//            functions::getRelativePose(pts_db, pts_q, K, R_qk, T_qk);
//
//            Eigen::Matrix3d R_qk_before_BA = R_q * R_ks[i].transpose();
//            Eigen::Vector3d T_qk_before_BA = T_q - R_qk_before_BA * T_ks[i];
//            T_qk_before_BA.normalize();
//
//            Eigen::Matrix3d R_qk_after_BA = new_R_q * new_R_ks[i].transpose();
//            Eigen::Vector3d T_qk_after_BA = new_T_q - R_qk_after_BA * new_T_ks[i];
//            T_qk_after_BA.normalize();
//
//            double r_dist_before = functions::rotationDifference(R_qk_before_BA, R_qk);
//            R_dist_before << r_dist_before << endl;
//            double t_dist_before = functions::getAngleBetween(T_qk_before_BA, T_qk);
//            T_dist_before << t_dist_before << endl;
//
//            double r_dist_after = functions::rotationDifference(R_qk_after_BA, R_qk);
//            R_dist_after << r_dist_after << endl;
//            double t_dist_after = functions::getAngleBetween(T_qk_after_BA, T_qk);
//            T_dist_after << t_dist_after << endl;
//        }
    }
