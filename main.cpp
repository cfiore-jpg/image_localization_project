...
int main() {
    ...
    for (const auto &scene: scenes) {
        ...
        for (int q = start; q < queries.size(); q++) {
            ...
            pose::adjustHypothesis(R_is, T_is, K_is, K_q, inliers_q, inliers_i,
...
//
//                double dist = sqrt(pow(pt.x - reproj2DFromEst.x, 2.) + pow(pt.y - reproj2DFromEst.y, 2.));
//                string title;
//                if (dist <= adj_threshold && double(p.second.size()) > .5 * f) {
//                    title = "INCLUDED";
//                } else {
//                    title = "NOT INCLUDED";
//                }
