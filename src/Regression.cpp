...
struct EpipolarConsensusError {
    EpipolarConsensusError(const vector<double>& cam_k, const vector<vector<double>>& matches_k)
    ...
    bool operator()(const T* const camera, T* residuals) const {
        ...
        T R_kq[9];
        R_kq[0] = T(cam_k[0])*camera[0] + T(cam_k[1])*camera[1] + T(cam_k[2])*camera[2];
        R_kq[1] = T(cam_k[3])*camera[0] + T(cam_k[4])*camera[1] + T(cam_k[5])*camera[2];
        R_kq[2] = T(cam_k[6])*camera[0] + T(cam_k[7])*camera[1] + T(cam_k[8])*camera[2];

        R_kq[3] = T(cam_k[0])*camera[3] + T(cam_k[1])*camera[4] + T(cam_k[2])*camera[5];
        R_kq[4] = T(cam_k[3])*camera[3] + T(cam_k[4])*camera[4] + T(cam_k[5])*camera[5];
        R_kq[5] = T(cam_k[6])*camera[3] + T(cam_k[7])*camera[4] + T(cam_k[8])*camera[5];

        R_kq[6] = T(cam_k[0])*camera[6] + T(cam_k[1])*camera[7] + T(cam_k[2])*camera[8];
        R_kq[7] = T(cam_k[3])*camera[6] + T(cam_k[4])*camera[7] + T(cam_k[5])*camera[8];
        R_kq[8] = T(cam_k[6])*camera[6] + T(cam_k[7])*camera[7] + T(cam_k[8])*camera[8];



        T t_kq[3];
        t_kq[0] = T(cam_k[9]) - (R_kq[0]*camera[9]+R_kq[1]*camera[10]*R_kq[2]*camera[11]);
        t_kq[1] = T(cam_k[10]) - (R_kq[3]*camera[9]+R_kq[4]*camera[10]*R_kq[5]*camera[11]);
        t_kq[2] = T(cam_k[11]) - (R_kq[6]*camera[9]+R_kq[7]*camera[10]*R_kq[8]*camera[11]);
        ...
        T K[9];
        K[0]=camera[12]; K[3]=0;          K[6]=camera[13];
        K[1]=0;          K[4]=camera[12]; K[7]=camera[14];
        K[2]=0;          K[5]=0;          K[8]=1;
        ...
        T l[3];
        for(int i = 0; matches_k.size(); i++)
        {
            l[0] = T(matches_k[i][0])*M[0] + T(matches_k[i][1])*M[1] + M[2];
            l[1] = T(matches_k[i][0])*M[3] + T(matches_k[i][1])*M[4] + M[5];
            l[2] = T(matches_k[i][0])*M[6] + T(matches_k[i][1])*M[7] + M[8];

            e = l[0]*T(matches_k[i][2]) + l[1]*T(matches_k[i][3]) + l[2];
    ...
    static ceres::CostFunction* Create(const vector<double>& cam_k,
                                       const vector<vector<double>>& matches_k) {
        return (new ceres::AutoDiffCostFunction<EpipolarConsensusError, matches_k.size(), 15>(
                new EpipolarConsensusError(cam_k, matches_k)));

    }

    vector<double> cam_k;
    vector<vector<double>> matches_k;
};