...
imageMatcher::imageMatcher(string indexed_image_list_file, string database_file_path, string feature_name, string matching_method)
{
    cout << "Construct" << endl;
    indexed_image_information_ = indexed_image_list_file;
    db_.load(database_file_path);
...
vector<pair<int, float> > imageMatcher::matching_one_image(cv::Mat image)
{
    vector<pair<int, float> > result_index;
    vector<vector<float> > feature;
    QueryResults results;
...
vector<pair<int, int> > imageMatcher::matching_images(cv::Mat image1, cv::Mat image2)
{
    vector< pair<int, int> > returnVector;
